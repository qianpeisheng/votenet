...
class ScannetDetectionDataset(Dataset):
    ...
    def __init__(self, split_set='train', num_points=20000,
        ...
        Args:
            ...
            num_points: int, number of points to sample in the point cloud
            use_color: bool, whether to use color as a feature
            use_height: bool, whether to use height as a feature
    ...
    def __getitem__(self, idx):
        ...
        if not self.use_color:
            ...
            if not self.debug:
                point_cloud[:,3:] = (point_cloud[:,3:]-MEAN_COLOR_RGB)/256.0 # the scene and the added object has different color ranges

        # only if training and the memory bank has some objects, insert objects
        ...
        if len(instance_bboxes) > MAX_NUM_OBJ:
            print(f'Scan {scan_name} num bbox {instance_bboxes.shape[0]} > {MAX_NUM_OBJ}, removing a few ...')
